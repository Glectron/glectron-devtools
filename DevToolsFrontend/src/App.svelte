<script lang="ts">
  ...
  async function refreshInjectorState() {
    await devtools.getInjectorState().then((val) => {
      ...
      if (!injectorStates[currentActiveInjector] && injectorCount > 0) {
...
<div class="select-none w-[100vw] h-[100vh]">
  <Resizable.PaneGroup direction="horizontal">
    <Resizable.Pane defaultSize={25} minSize={5}>
      <div class="h-full flex flex-col gap-2 p-2">
        {#each Object.values(injectorStates) as injectorState (injectorState.ProcessId)}
          ...
          <button
            ...
            onclick={() => {
              showSettings = false;
              currentActiveInjector = injectorState.ProcessId;
            }}
          >
        ...
        <button
          class="flex items-center text-left border-1 p-2 cursor-pointer rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 overflow-hidden"
          class:shadow-sm={showSettings}
          onclick={() => showSettings = !showSettings}
          >
          <Settings class="mr-2 shrink-0"/>Settings
        </button>
      </div>
    </Resizable.Pane>
    <Resizable.Handle />
    <Resizable.Pane defaultSize={75} minSize={5}>
      {#if showSettings}
        <SettingsPanel />
      {:else}
        {#if injectorCount > 0}
